...
$result = mysql_query("SELECT Price 
    ...
    FROM bharat_petroleum )) AND Date=
      (SELECT max(Date)
       FROM bharat_petroleum)");
...
$result1 = mysql_query("SELECT Price 
    FROM bharat_petroleum
    WHERE Time=
      (SELECT max(Time) FROM bharat_petroleum
      WHERE Date=(SELECT max(Date) FROM bharat_petroleum
       Where Date < ( SELECT max(Date) from bharat_petroleum)))
       AND Date=
      (SELECT max(Date) FROM bharat_petroleum
       Where Date < ( SELECT max(Date) from bharat_petroleum))");
